...
?>
<?php echo form_open($form_action, 'class="form-horizontal transfer_form" id="transfer_form"');?>
<input type="hidden" name="action_mode" value="<?=$action_mode?>" />
<input type="hidden" name="dispatch_id" value="<?=$dispatch_id?>" />
<input type="hidden" name="ref_dispatch_id" value="<?=$ref_dispatch_id?>" />
...
<div class="panel-body" id="panel-body-container">
		...
		<table class="table table-striped" width="100%" id="container_table">
			...
			if(isset($all_boxes) && !empty($all_boxes)){
                        ...
                        foreach($all_boxes as $box_data){							
							...
							$checked = '';
							if($dispatch_id == $box_data['box_dispatch_id']){
                            ...
                            if(is_array($box_items) && !empty($box_items) && empty($sale_id)){
								...
								echo transfer_box_list_html($i, $box_data, $dataset, 'transfer', $checked);
                            }                            
                        	$i++;
						}
                    }
	...
	<div class="form-group margin-top-15">
		...
		<div class="col-sm-3 margin-top-5">
            <button type="button" class="btn btn-success transfer_btn" data-url="<?=$form_action?>"> <i class="fa fa-truck"></i> Update Transfer</button>&nbsp;
            <a href="<?=site_url('dispatch/dispatched/view/'.$dispatch_id)?>" class="btn btn-danger">Cancel</a>
		</div>
	</div>
</div>
<?php echo form_close();?>
