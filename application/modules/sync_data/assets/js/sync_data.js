function ajaxRequest(url, ele, page_number){	
	...
	var synced_rows = $(ele).data('srows');
	var sync_date = $('#sync_date').val();
	var limit = $('#data_limit').val();
	if(sync_date == ''){
		sync_date = $(ele).data('last_sync');
	}
		
	$.ajax({
		url: url,
		type:'GET',
		data: {data_type:data_type, sync_date:sync_date, page: page_number, limit:limit, srows:synced_rows},
		//async:false,
