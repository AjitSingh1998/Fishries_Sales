    ...
    <div class="col-sm-10 col-sm-offset-1">
      <div class="panel panel-white" id="panel_container">
        <div class="panel-heading border-light light-bg panel-sale">
            ...
            <button type="button" class="btn btn-warning btn-xs depot_edit_mode" data-url="<?=site_url('dailysale/ajax_depot_edit_mode/'.$sale_id)?>">Edit</button>
            <button type="button" class="btn btn-warning btn-xs update_rate" data-url="<?=site_url('dailysale/update_rate/'.$sale_id)?>">Update Rate</button>
            <?php } ?>
          </h3>
          <?php if($sale_id){ ?>
          	<input type="hidden" id="client_id" value="<?=$sale_data['client_id']?>" />
          	<input type="hidden" id="market_id" value="<?=$sale_data['market_id']?>" />
          	<div class="text-center"><?=$sale_data['market_name']?> / <?=$sale_data['client_name']?> <?=($sale_data['client_mobile']) ? '('.$sale_data['client_mobile'].')' : '' ?> / <?=$sale_data['invoice_number']?> / <?=get_datetime('d-m-Y h:i a', $sale_data['sale_date'])?>
            
              &nbsp;&nbsp;&nbsp;&nbsp;&nbsp;
             <button type="button" class="btn btn-info btn-xs open_aside_modal" data-style="bottom" data-toggle="tooltip" data-title="Import Excel Data" data-url="<?=site_url('excel_import/import/depot_sale/'.$sale_id)?>"> 
		  ...
		  <?php echo form_open('dailysale/depot/'.$action_mode, 'class="form-horizontal depot_sale_form" id="depot_sale_form"');?>
          ...
          <div class="row">
            <div class="col-sm-8 sale-response"></div>
          </div>
          ...
          <div class="form-group">
            ...
            <div class="col-sm-2 margin-top-15"> <br />
              <button type="button" class="btn btn-primary save_depot_sale_data">Save</button>
              &nbsp; </div>
          </div>
          <?php } ?>
          <?php echo form_close();?>
        ...
        <div class="panel-body" id="panel-body-container">
          <table class="table listinge_header" width="100%" id="header_table">
            ...
            <tbody class="tbody_sale_item">
              ...
              <tr>
                <td><?=$i?></td>
                <td><input type="text" readonly="readonly" value="<?=$item['carret_number']?>" class="form-control disabled input-sm si_carret_number" /></td>
                ...
                <?php if($action_mode == 'add' || $action_mode == 'edit'){ ?>
                <td>
                    <a href="javascript:void(0);" class="btn btn-primary btn-xs edit_sale_item" data-sale_id="<?=$item['sale_id']?>" data-sale_item_id="<?=$item['id']?>" data-carret_number="<?=$item['carret_number']?>"> <i class="fa fa-pencil"></i> </a>
                    <a href="javascript:void(0);" class="btn btn-danger btn-xs delete_sale_item" data-sale_id="<?=$item['sale_id']?>" data-sale_item_id="<?=$item['id']?>" data-carret_number="<?=$item['carret_number']?>"> <i class="fa fa-trash"></i> </a>
                </td>
                <?php } ?>
              </tr>
              <?php $i++; }} ?>
            ...
            <tfoot>
                ...
                <form id="sale_item_form">
                  <tr id="tr_sale_item">
                    ...
                    <td><button type="button" class="btn btn-success btn-sm save_depot_item" data-toggle="tooltip" data-title="Save Item"> <i class="fa fa-save"></i></button></td>
