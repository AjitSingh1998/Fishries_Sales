...
$(document).ready(function(e) {
	...
	$(document).on('click', '.save_point_sale_data',  function(){
	...
	$('.point_edit_mode').on('click', function(){
	...
	//Save row
	$('.save_point_item').on('click', function(){
		$this = $(this)
		$this.prop('disabled', true);
		var tr 				= $('#tr_sale_item');
		var item_form 		= $('#sale_item_form');
		var fish_code 		= $('#fish_code').val();
		var fish_weight 	= $('#fish_weight').val();
		var fish_rate 		= $('#fish_rate').val();	
		
		if(fish_code && fish_weight > 0 && fish_rate > 0){
			var datatosend = {};
			...
			datatosend[csrf_token_name] = csrf_token_value;
			$.ajax({
				url : site_url+'dailysale/ajax_save_point_item', 
				type : "POST",
	...
	$(document).on('click', '.delete_sale_item', function(){
		...
		if(confi){
			...
			$.ajax({
				url : site_url+'dailysale/ajax_delete_point_item', 
				type : "POST",
				beforeSend: function(){$('.ajax-response').html(''); show_loader($('#panel_container'));},
				data : datatosend,
				success: function(response, status, xhr){
	...
	$('.save_n_back').on('click', function(){
		...
		datatosend[csrf_token_name] 	= csrf_token_value;
		$.ajax({
			url : site_url+'dailysale/ajax_save_point_summary', 
			type : "POST",
			beforeSend: function(){$('.ajax-response').html(''); show_loader($('#panel_container'));},
			data : datatosend,
			success: function(response, status, xhr){
