...
class grocery_CRUD_Layout extends grocery_CRUD_Model_Driver
{
	...
	protected function showList($ajax = false, $state_info = null)
	{
		...
		$data->add_button_class		= $this->add_button_class;
		$data->add_button_text		= $this->add_button_text;
		$data->add_button_icon		= $this->add_button_icon;
		
		$data->edit_url				= $this->getEditUrl('', $this->edit_url_path);
		
		$data->read_button_class	= $this->read_button_class;
		$data->edit_button_class	= $this->edit_button_class;
		$data->edit_button_text		= $this->edit_button_text;
		$data->edit_button_icon		= $this->edit_button_icon;
		
		$data->delete_url			= $this->getDeleteUrl('', $this->delete_url_path);
		...
		$data->action_dependency	= $this->action_dependency;
		$data->custom_action_buttons= $this->customActionButtons($data->list); 
		...
		$data->display_summary 		= $this->display_summary;
		...
		$data->unset_common_search	= $this->unset_common_search;
	...
	protected function _export_to_excel($data)
	{
		...
		$filename = $class."_".$method."_report-".get_date("d-M-Y_h:i-A").".xlsx";
		...
		foreach($data->columns as $column){
			...
			$objPHPExcel->getActiveSheet()->SetCellValue($clmn.'1', $column->display_as);
			$clmn++;
		}		
		$rwn=2;		
		foreach($data->list as $num_row => $row){
			$clmn='A';
			foreach($data->columns as $column){				
				$objPHPExcel->getActiveSheet()->SetCellValue($clmn.$rwn, $this->_trim_export_string($row->{$column->field_name}));
				$clmn++;				
			}
			$rwn++;			
		}
	...
	// This function added by Manoj	
	protected function customActionButtons($list){
		if(!empty($this->custom_action_buttons) && $this->custom_action_buttons != null){			
			$custom_action_button = array();
			$primary_key = $this->get_primary_key();
			foreach($list as $num_row => $row)
			{
				$custom_action_button[$num_row] = call_user_func($this->custom_action_buttons, $row->$primary_key, $row);
			}
			return $custom_action_button; 		
		}
		return null;
	}
	...
	protected function load_js_chosen()
	{
		...
		$this->set_css($this->default_css_path.'/jquery_plugins/select2/css/select2.min.css');
		$this->set_js_lib($this->default_javascript_path.'/jquery_plugins/select2/js/select2.min.js');
	}
...
class grocery_CRUD_States extends grocery_CRUD_Layout
{
	...
	protected function getEditUrl($primary_key = null, $custome_url='')
	{
		if($custome_url === null){
			if($primary_key === null)
...
class Grocery_CRUD extends grocery_CRUD_States
{
	...
	protected $action_dependency	= array();
	protected $custom_action_buttons= NULL;
	
	protected $field_without_sorter	= array();
	protected $display_summary		= array();
	
	protected $read_url_path		= null;
	protected $add_url_path			= null;
	protected $add_button_text		= null;
	protected $add_button_class		= null;
	protected $add_button_icon		= null;
		
	protected $edit_url_path		= null;
	protected $delete_url_path		= null;
	
	protected $read_button_class	= null;
	protected $edit_button_text		= null;
	protected $edit_button_class	= null;
	protected $edit_button_icon		= null;
	...
	protected $unset_common_search  = false;
	...
	// WEBROOTDIR is defined in root Index.php file to access root directory path
	...
	/**
	 * Set The Custome read BUTTON CLASS to add Data
	 * @access	public
	 * @param	string
	 * @return	void
	 */
	public function set_read_button_class($read_button_class = '')
	{
		$this->read_button_class = $read_button_class;
		return $this;
	}
	...
	/**
	 * Set The Custome ADD BUTTON TEXT to add Data
	 * @access	public
	 * @param	string
	 * @return	void
	 */
	...
	/**
	 * Set The Custome ADD BUTTON CLASS to add Data
	 * @access	public
	 * @param	string
	 * @return	void
	 */
	public function set_add_button_class($button_class = '')
	{
		$this->add_button_class = $button_class;
		return $this;
	}
	 ...
	 * @return	void
	 */
	public function set_add_button_icon($button_icon = '')
	{
		$this->add_button_icon = $button_icon;
		return $this;
	}
	...
	/**
	 * Set The Custome DELETE URL Path to edit Data
	 * @access	public
	 * @param	string
	 * @return	void
	 */
	public function set_delete_url_path($url = '')
	{
		$this->delete_url_path = $url;
		return $this;
	} 
	
	/**
	 * Set The Custome edit BUTTON TEXT to add Data
	 * @access	public
	 * @param	string
	 * @return	void
	 */
	public function set_edit_button_text($edit_button_text = '')
	{
	...
	/**
	 * Set The Custome edit BUTTON CLASS to add Data
	 * @access	public
	 * @param	string
	 * @return	void
	 */
	public function set_edit_button_class($edit_button_class = '')
	{
		$this->edit_button_class = $edit_button_class;
		return $this;
	}
	
	/**
	 * Set The Custome edit BUTTON ICON to add Data
	 * @access	public
	 * @param	string
	 * @return	void
	 */
	public function set_edit_button_icon($edit_button_icon = '')
	{
		$this->edit_button_icon = $edit_button_icon;
		return $this;
	}
	...
	public function unset_search($column_name = NULL)
	{
		if(!is_array($column_name) && $column_name != NULL){
			$unset_search_columns = array($column_name);
		}else if(is_array($column_name) && $column_name != NULL){
			$unset_search_columns = $column_name;
		}else{
			$unset_search_columns = true;
		}
		
		$this->unset_search = $unset_search_columns;
		
		return $this;
	}
	
	public function unset_common_search()
	{
		$this->unset_common_search = true;
		return $this;
	}
	 ...
	 * @param $url_callback
	 */
	public function add_action( $label, $image_url = '', $link_url = '', $css_class = '', $url_callback = null, $url_target = null)
	{
	 ...
	 * @param $css_class
	 * @param $icon_class
	 * @param $column_name : column name is table's column on which will perform action.
	 * @param $image_url
	 * @param $url_callback
	 */
	public function add_bulk_action($label, $link_url = '', $css_class = '', $icon_class = '', $column_name = '', $image_url = '', $url_callback = null)
	{
		$unique_id = substr($label,0,1).substr(md5($label.$link_url),-8); //The unique id is used for class name so it must begin with a string
		$this->bulk_actions[$unique_id]  = (object)array(
			'label' 		=> $label,
			'link_url'		=> $link_url,
			'css_class' 	=> $css_class,
	...
	/**
	 *
	 * Set action button dependency based on specific column
	 * @param string $table_column_name	 
	 */
	
	public function set_action_dependency($table_column_name, $action_menu_array, $callback_func = NULL){	
		$uniqid = uniqid();	
		$this->action_dependency[$uniqid] = array('column_name' => $table_column_name, 'action_menu' => $action_menu_array, 'callback_func'=>$callback_func);
	}	
	
	/**
	 *
	 * Set custome action buttons through callback function
	 * @param string $table_column_name	 
	 */
	
	public function set_custom_action_buttons($callback_function = NULL){	
		$this->custom_action_buttons = $callback_function;	
	}
	...
	public function set_field_upload($field_name, $upload_dir = '', $allowed_file_types = '')
	{
		...
		if (!is_dir($this->root_url.$upload_dir)) {
			throw new Exception("It seems that the folder \"".$upload_dir."\" for the field name
...
function check_dependency($row, $action_menu_data, $menu_label = NULL){
	if(isset($row) && isset($action_menu_data) && !empty($action_menu_data) && !empty($menu_label)){	
		foreach($action_menu_data as $menues){			
			if(array_key_exists($menues['column_name'], $row)){
				$column_value = $row->$menues['column_name'];
				$action_menu = $menues['action_menu'];
				$callback_func = $menues['callback_func'];				
				if(is_array($callback_func) && !empty($callback_func) && array_key_exists($column_value, $action_menu) && $action_menu[$column_value] ==  $menu_label){	
					$result = call_user_func($callback_func, $row, array($menues['column_name'] => $menues['action_menu']), $menu_label);
					if($result){
						return false;
					}
				}
				
				if(array_key_exists($column_value, $action_menu)){						
					if($action_menu[$column_value] ==  $menu_label){
						return false;
					}
				}
