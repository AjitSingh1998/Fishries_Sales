...
(function ($) {
    ...
    Datagrid.prototype.datagridInit = function () {
		...
		if(csrf_token_name !== undefined && csrf_token_name.length > 1 && this.csrf_field == null){
			this.csrf_field = {
                name: csrf_token_name, //$('#gcrud-search-form>div:first>input[type=hidden]').attr('name'),
                value: csrf_token_value //$('#gcrud-search-form>div:first>input[type=hidden]').val()
            };
    ...
    Datagrid.prototype.listenerExportButton = function () {
        ...
        datagrid_object.gcrud_container.find('.gc-export').click(function () {
			...
			var commonSearch = datagrid_object.gcrud_container.find('.search-input');		
            if (commonSearch.length > 0 && commonSearch.val() !== '' && commonSearch.val() !=='undefined') {
    ...
    Datagrid.prototype.SearchAndOrderingTrigger = function () {
        ...
        if (search_all_input.val() !== '' && search_all_input.val() !== undefined) {
        ...
        $.ajax({
            ...
            success: function (result) {
				...
				//this method is calling to bind custom delete button
				datagrid_object.triggerCustomBulkAction();
    ...
    Datagrid.prototype.hideShowDeleteButton = function () {
        ...
        if (this.gcrud_container.find('.select-row:checked').length > 0) {
            this.gcrud_container.find('a.delete-selected-button').removeClass('hidden');
        } else {
            this.gcrud_container.find('a.delete-selected-button').addClass('hidden');
        }
		
		if (this.gcrud_container.find('.select-row:checked').length > 0) {
            this.gcrud_container.find('div.bulk_action').removeClass('hidden');
        } else {
            this.gcrud_container.find('div.bulk_action').addClass('hidden');
        }
    };
	
	Datagrid.prototype.triggerCustomBulkAction = function (){
		var datagrid_object = this;
		datagrid_object.gcrud_container.on('click', '.triggerBulkDelete', function(event){
			event.preventDefault();
														
			var $this = $(this);
			$this.closest('tr').find('input[type="checkbox"]').prop('checked', true);
			var CurrentButtonUrl = $this.attr('data-url');
						
			var bulkActionButtons = datagrid_object.gcrud_container.find('.bulk_action_button');
			if(bulkActionButtons.length){
				bulkActionButtons.each(function(index, element) {
					var bulkActionUrl = $(this).attr('data-url');
					if(bulkActionUrl == CurrentButtonUrl){
						$(this).trigger('click');
					}
				});
			}
	...
	Datagrid.prototype.performBulkAction = function (){
		var datagrid_object = this;
        datagrid_object.gcrud_container.find('.bulk_action_button').on('click', function(){
			...
			var my_model;
			$('.bulk-action-confirmation').on('show.bs.modal', function (e) {
				my_model = $(this);
			  	my_model.find('.modal-title').text(title + ' selected items');
				my_model.find('.bulk-action-confirmation-button').text(title);
			});
			$('.bulk-action-confirmation').modal('show').one('click', '.bulk-action-confirmation-button', function() {
				
				$('.bulk-action-confirmation').modal('hide');
				...
				data_to_send = {
                    items: delete_selected, table_name:table, column_name:column, primary_key:primary_key
                };
