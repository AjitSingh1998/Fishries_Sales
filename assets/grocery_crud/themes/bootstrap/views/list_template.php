    ...
    <div class="container gc-container">
        ...
        ?></div>
 		...
 		<div class="row">
                    ...
                    <?php echo form_open("", 'method="post" autocomplete="off" id="gcrud-search-form"'); ?>
                        <div class="header-tools">
                            <?php if(!$unset_add){?>
                                <div class="floatL t5">
                                	<?php
                                    	$button_text = isset($add_button_text) ? $add_button_text : $this->l('list_add') . ' ' . $subject;
										$button_class = isset($add_button_class) ? $add_button_class : 'btn btn-default';
										$button_icon = isset($add_button_icon) ? $add_button_icon : 'fa fa-plus';
									?>
                                    <a class="<?php echo $button_class; ?>" href="<?php echo $add_url?>">
        					...
        					<thead>
        						<tr>
                                    ...
                                    <?php foreach($columns as $column){?>                                    	
                    					<?php if(in_array($column->field_name, $field_without_sorter)){?>
                                ...
                                <?php if (!empty($actions) || !empty($row->action_urls) || !$unset_edit || !$unset_read  || !$unset_delete  || count($bulk_actions)>0) {?>
                                         ...
                                         <td style="border-left: none;min-width:140px;">
                                            <div class="floatL">
                                                <?php if(count($bulk_actions) <= 0 && !$unset_delete){?>
                                                ...
                                                <?php }else{?>
                                                <div class="btn-group dropdown bulk_action hidden">
                                                    <button type="button" class="btn btn-default dropdown-toggle" data-toggle="dropdown">Action <span class="caret"></span></button>                
                                                    <ul class="dropdown-menu">
                                                        <?php if(!$unset_delete){?>
                                                        <li><a href="javascript:void(0);" class="delete-selected-button">
                                                            <i class="fa fa-times text-danger"></i>
                                                            <span class="text-danger"><?=$this->l('list_delete')?></span>
                                                        </a></li>
                                                        <?php }?>
                                                        <?php											
                                                        foreach($bulk_actions as $ab){
                                                            echo '<li><a class="bulk_action_button'.$ab->css_class.'" href="javascript:void(0);" data-title="'.$ab->label.'" data-table="'.$basic_db_table.'" data-column="'.$ab->column_name.'" data-primary_key="'.$primary_key.'" data-url="'.$ab->link_url.'">'.($ab->icon_class ? '<i class="'.$ab->icon_class.'"></i> ':'').$ab->label.'</a></li>';														
                                                        }
                                                        ?>
                                                    </ul>
                                                </div>
								...
								<?php foreach($columns as $column){?>
                                    <td>
                                        <?php 
                                        if(isset($unset_search) && is_array($unset_search)){
                                            if(!in_array($column->field_name, $unset_search)) { ?>
                                        ...
                                        <?php }else{ ?>
											<input type="text" class="form-control floatL" readonly="readonly" placeholder="Search Disabled" />
										<?php } ?>
                                        <?php }else{ ?>
                                        <input type="text" class="form-control searchable-input floatL" placeholder="Search <?php echo $column->display_as; ?>" name="<?php echo $column->field_name; ?>" />
                                        <?php
            ...
            <div class="bulk-action-confirmation modal fade">
                <div class="modal-dialog">
                    <div class="modal-content">
                        <div class="modal-header">
                            <button type="button" class="close" data-dismiss="modal" aria-hidden="true">&times;</button>
                            <h4 class="modal-title"><?php echo 'Bulk action'; ?></h4>
                        </div>
                        <div class="modal-body">
                            <p><?php echo 'Are you sure to perform this action?'; ?></p>
                        </div>
                        <div class="modal-footer">
                            <button type="button" class="btn btn-default" data-dismiss="modal">
                                <?php echo $this->l('form_cancel'); ?>
                            </button>
                            <button type="button" class="btn btn-danger bulk-action-confirmation-button">
                                <?php echo 'Perform action'; ?>
                            </button>
                        </div>
                    </div>
                </div>
            </div>
            <!-- End Bulk action confirmation dialog -->
