...
$(document).ready(function() {
	...
	$('a.add_break_hour').click(function(){
		...
		course_input.append('<select name="'+day_name+'[break][course][]" class="form-control">'+course_name+'</select>');
...
$('#add_more_phone').click(function(){
	var parent_el = $(this).closest('.parentClone');
	var $cloned = parent_el.clone();
	$cloned.find("input").val("");
	var remove_button = $('<label for=""></label><button type="button" id="remove_more_phone" class="btn btn-danger btn-xm" style="display:inherit; margin-top: 4px;"> <i class="fa fa-trash"></i> </button>');
	$cloned.find('#actionHandler').html(remove_button);
	remove_button.click(function(){
	$(this).closest('.parentClone').remove();
	});
	parent_el.after($cloned);
});

$('#remove_more_phone').click(function(){
	$(this).closest('.parentClone').remove();
});
