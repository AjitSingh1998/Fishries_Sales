...
var Main = function () {
	...
	var navbarHandler = function () {
		...
		var horizontalNavbar = $('.horizontal-nav-container');

		var horizontalCollapseButton = $('#horizontal-menu-toggler');
		if (isSmallDevice()) {
			horizontalNavbar.css({
				height: pageHeight
			});
		} else {
			horizontalNavbar.css({
		...
		$(document).on("mousedown touchstart", horizontalToggleNavbar);
		function horizontalToggleNavbar(e) {
			if (horizontalNavbar.has(e.target).length === 0 && !horizontalNavbar.is(e.target) && horizontalCollapseButton.has(e.target).length === 0 && !horizontalCollapseButton.is(e.target) && horizontalNavbar.closest(".horizontal-menu").hasClass("collapse in")) {
				horizontalCollapseButton.trigger("click");
			}
		}
	...
	var settingsHandler = function () {
		...
		$('#skin_color').attr("href", template_path + "css/themes/" + appSetting.theme + ".css");
		...
		$('input[name="setting-theme"]').change(function () {
			...
			$('#skin_color').attr("href", template_path + "css/themes/" + selectedTheme + ".css");
		...
		function switchLogo(theme) {
			...
			switch (theme) {
				...
				case "lyt4-theme-4":
				case "lyt4-theme-5":
					$(".navbar-brand img").attr("src", template_path + "images/logo.png");
					break;

				default:
					$(".navbar-brand img").attr("src", template_path + "images/logo2.png");
					break;
			}
		}
	...
	$('#reset-layout').on('click', function () {
		...
		$('#skin_color').attr("href", template_path + "css/themes/lyt4-theme-1.css");
		switcheryHandler();
		settingsHandler();
		setTimeout(function () {
...
function show_loader(element) {
	var $this = element, csspinnerClass = 'csspinner', panel = element, spinner = $this.data('spinner') || "load1";
	panel.addClass(csspinnerClass + ' ' + spinner);
}
